...
def read_shape_coordinates(shape_file_path):
    utmcord = []
    ...
    elif file_suffix == '.geojson':
        df = gpd.read_file(shape_file_path)
        if df.geometry[0].type == 'Point':
            reverse_coords = True
            data = df.values
        data = df.geometry[0].exterior.coords
    elif file_suffix == '.kml':
        df = gpd.read_file(shape_file_path, driver='KML')
        data = df.geometry[0].exterior.coords 
    for row in data:
        if reverse_coords:
            yield (float(row[1]), float(row[0]))
        else:
